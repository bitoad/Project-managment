...
> Single source of truth for all UI decisions in this repository.
> Always read this file before creating pages, creating components, modifying layouts, or generating Tailwind classes. Never invent a new design language.

---

# PART 1 — Industrial Engineering Dashboard (Active System)

```yaml
version: alpha
name: Industrial-Engineering-Dashboard
description: >
  A minimal, high-density, professional dashboard system for EPC / PM
  operations (Kanban, Gantt, Risk Matrix, Item Master/BOQ, Cost Log, S-Curve).
  Optimized for information density and fast scanning over decoration.
```

## Colors

> Đồng bộ với `src/theme.css` (hệ thống thực tế đang chạy). Brand = xanh dương `#2F5CE0` (đại diện action), xanh lá `#166534` (brand Đại Lâm Mộc, dùng cho accent/success tone). Không dùng PART 2.

```yaml
colors:
  primary: "#2F5CE0"
  primary-hover: "#1F6FD6"
  primary-tint: "rgba(47, 92, 224, 0.10)"
  primary-grad: "linear-gradient(135deg, #0F5132 0%, #2F9E6E 100%)"

  success: "#2F9E6E"
  success-hover: "#1A8A5C"
  success-tint: "rgba(47, 158, 110, 0.12)"

  danger: "#C1502E"
  danger-hover: "#A63F24"
  danger-tint: "rgba(193, 80, 46, 0.10)"

  warning: "#D9A441"
  warning-hover: "#C4902F"
  warning-tint: "rgba(217, 164, 65, 0.14)"

  background: "#F4F7F5"
  surface: "#FFFFFF"
  surface-alt: "#FBFCFB"     # alternating table row

  border: "#E4EDE8"
  border-strong: "#D3E2DA"

  text-primary: "#1B2B22"
  text-secondary: "#6B8377"
  text-muted: "#9AA8A0"
...
```yaml
rounded:
  sm: 8px       # input / button / tag
  md: 12px      # base radius (card / section)
  lg: 14px
  full: 9999px
```
...
```yaml
shadow:
  soft: "0 4px 12px rgba(16, 24, 40, 0.06), 0 2px 6px rgba(16, 24, 40, 0.04)"
  raised: "0 12px 32px rgba(16, 24, 40, 0.12)"
```

## Icons

Lucide React — 16px inline / 20px standalone. Stroke width 1.75.
...
animation:
  duration: 150-200ms
  easing: "ease-out"
  scope: "state transitions only (hover, active, expand/collapse) — no decorative motion"
```

## Components

> Class chuẩn thực tế nằm trong `src/theme.css` / `src/index.css` (prefix `.ds-`, `.dash-`, `.btn-`, `.ant-*` được override). Dùng class thay vì inline style.

**`button-primary`** — Class `.btn.btn-primary` (hoặc `.ant-btn-primary`). Background `{colors.primary}`, text `{colors.text-on-primary}`, `{typography.body-strong}`, rounded `{rounded.sm}`, height 38px, padding 0 20px. Hover: `{colors.primary-hover}`. Active: `transform: translateY(1px)`.

**`button-secondary`** — Class `.btn.btn-outline`. Background transparent, text `{colors.primary}`, 1px solid `{colors.primary}`, rounded `{rounded.sm}`. Hover: `{colors.primary-tint}`.

**`button-danger`** — Class `.btn.btn-danger`. Background `{colors.danger}`, text `{colors.text-on-primary}`. Reserved for destructive actions.

**`status-badge`** — Dùng Ant `Tag` + helper `statusColor` / `priorityColor` / `procStatusColor` trong `src/components/helpers.js` (trả Ant Tag token: `blue/red/green/orange/...`). Pill, rounded `{rounded.full}`, `{typography.caption}` weight 500. Không dùng free-text color coding.

**`table`** — Class `.ds-table` / `.ds-table-premium` / `.dash-table`. Sticky header (background `{colors.surface-alt}`, border-bottom 2px `{colors.border-strong}`, `{typography.label}` uppercase). Compact rows (padding 8–12px). Alternating row background. Row hover: `{colors.primary-tint}`.

**`form-input`** — Ant `Input`/`InputNumber`/`Select`/`Picker` override trong theme.css (rounded `{rounded.sm}`). Label `{typography.label}` above, 4px gap. Focus: `box-shadow: 0 0 0 3px {colors.primary-tint}` + border `{colors.primary}`. Error: border `{colors.danger}`.

**`card`** — Class `.ds-card` / `.dash-card` / `.ds-section`. Background `{colors.surface}`, 1px solid `{colors.border}`, rounded `{rounded.md}`, padding `{spacing.lg}`, shadow `{shadow.soft}`.

**`kpi-card`** — Class `.ds-kpi` (component `StatCard.jsx`). Top: `{typography.label}` in `{colors.text-muted}`. Middle: value `{typography.display}`. Bottom: delta/trend `{typography.caption}` colored by `{colors.success}` or `{colors.danger}`.

**`sidebar-nav`** — Class `.app-sider` (Ant `Layout.Sider`), gradient tối `var(--sidebar-gradient)`, width 240px (collapsed icon-only). Active item: background `{colors.primary-grad}`, text `{colors.text-on-primary}`.

**`chart-container`** — Class `.ds-chart-card` / `.chart-container`. Recharts, palette from `{colors.chart-palette}` in order. Background `{colors.surface}`, rounded `{rounded.md}`, padding `{spacing.md}`. Gridlines `{colors.border}` low opacity.

## Do's and Don'ts

### Do
- Keep information density high — compact rows, tight spacing.
- Use `{colors.chart-palette}` in fixed order across all charts for visual consistency.
- Use `Tag` + helper `statusColor`/`priorityColor` for all Kanban/Risk/PO status — never free-text color coding.
- Keep animation to state transitions only (150–200ms).
- Reference tokens via `var(--...)` in CSS (theme.css) — never inline hex/px in JSX.

### Don't
- Don't introduce accent colors outside `{colors.primary}` / `success` / `danger` / `warning` (purple `#8B5CF6`, sky `#0EA5E9` chỉ dùng hạn chế cho brand gradient).
- Don't use heavy shadows on table rows or form inputs — reserve `{shadow.soft}` for cards.
- Don't mix font families — Inter only.
- Don't round tables or sticky headers — corners stay square except the outer card wrapper.

## Responsive Behavior

| Breakpoint | Width | Key Changes |
|---|---|---|
| Mobile | ≤ 640px (`!screens.sm`) | Sidebar → Drawer (slide-in) + hamburger; tables scroll horizontally / card-list |
| Tablet | 641–1024px (`screens.md`..`<lg`) | Sidebar full or collapsed icon-only; tables scroll horizontally |
| Desktop | 1025–1440px | Full sidebar + table layout |
| Wide | ≥ 1441px | `dash-shell` max-width none, padding 24px |

> Lưu ý: hiện tại mobile dùng Drawer thay vì bottom-nav (xem audit ISSUE-04). Có thể nâng cấp thành bottom-nav sau.

## Iteration Guide

1. Reference tokens via `var(--...)` in `theme.css` — never inline hex/px in JSX `style={{}}`.
2. New component variants live as sibling class under `Components` (`.ds-*` / `.dash-*`).
3. Default / Hover / Active / Focus / Error states — keep hover transitions 150–200ms.
4. When adding a status type, extend `statusColor`/`priorityColor` in `helpers.js` (Ant Tag token) rather than inventing a color.

## Known Gaps

- Dark theme (graphite) variant là exploration, chưa formalized.
- Bottom-nav cho mobile (≤640px) chưa có — đang dùng Drawer (audit ISSUE-04).
- Empty/loading states: đã có `shared/EmptyState.jsx`, `shared/PageLoader.jsx`, `.ds-skeleton` nhưng chưa áp dụng đồng loạt mọi Table/Chart.

---

# PART 2 — Apple-design-analysis (Reference Only, Not Active)

> ⚠️ This section is a separate, unrelated design system (Apple marketing/product-page analysis). It is kept here for reference/inspiration only and must **not** be mixed with Part 1 tokens. Do not pull colors, typography, or components from this section into the Industrial Dashboard system.
